...
function Remove-CustomActionForSPFxExt ([string]$extensionName, [string]$url, $clientContext) {
    ...
    foreach ($action in $actionsToRemove) {
        Remove-PnPCustomAction -Identity $action.Id -Force
